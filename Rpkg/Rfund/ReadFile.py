...
def one_image(input_path, color_type='Gray'):
    ...
    入力されたパスの画像を読み出します。(注意: 一枚のみ)
        ...
        :param input_path: 画像のパス
        :param: color_type: カラータイプColor or RGB or Gray(デフォルトはGray)
...
def directory_images(dirpath, imagesize, type_color):
    ...
    このとき、画像データはnumpyの形式に変更します。(注意: フォルダ内の画像すべて)
        ...
        :param dirpath: 画像のパス。
        :param imagesize: 整形したい画像サイズの大きさ。
        :param color_type: カラータイプ。ColorまたはGray。

        :returns: numpy形式にした画像リストと、
        :returns: ファイル名リストを返します。
    """
    imglist = []
    exclude_prefixes = ('__', '.') 
...
def directory_mc_images(dirpath, imagesize, type_color):
    ...
    このとき、画像データはnumpyの形式に変更します。(注意: フォルダ内の画像すべて)
        ...
        :returns: numpy形式にした画像リストと、ファイル名リストを返します。
    """
    imglist = []
    exclude_prefixes = ('__', '.') 
...
def directory(input_dir, include_ext=[], exclude_ext=[]):
    ...
    ファイル名のリストを返します
    #
        :param input_dir: ディレクトリの名前
        :param include_ext: 読み込みたい拡張子のリスト
        :param exclude_ext: 除外した拡張子のリスト
        注) 拡張子の前には.を入れる(例: '.png')

        :returns: ディレクトリにあるファイル名
    """
    path_list = []
    name_list = []
