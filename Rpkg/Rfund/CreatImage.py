...
def mfeatureImages(marray, LEARN_CASES, TEST_CASES, lineDir, datasetDir, rootDir, testmode=False):
    ...
    一枚のmチャンネルの画像に変換する
    #
        :param marray: m個の特徴画像の名前(String)
        :param LEARN_CASES: 学習のグループ(String)
        :param TEST_CASES: テストのグループ(String)
        :param lineDir: 椎間板のラベル画像があるディレクトリ
        :param datasetDir: フィルタをかけた椎間板画像があるディレクトリ
        :param rootDir: 作成された画像を保存するディレクトリ
    """
    m = len(marray)
