...
# ★★★出力ファイル設定 
featureRootDir = image_root_dir + 'feature-dataset/'
...
# 特徴一覧をリストとして取得
inputfeature_list = list(map(str, InputFeature))    # ini_filter_dfの代わり
...
# mask_g1, mask_g1_filenames = read_csv_image(mask_dir, group1_filenames)
...
# mask_g2, mask_g2_filenames = read_csv_image(mask_dir, group2_filenames)
...
# mask_g3, mask_g3_filenames = read_csv_image(mask_dir, group3_filenames)
...
# def get_mask_data(group_num):
#     """
#     ラベルを取得
#     
#     :param group_num: グループの番号(ここでは1~3)
    
#     :returns: mask: 画像numpyのlist
#     :returns: mask_filnames: ファイル名
#     """
#     if group_num == 1:
#         mask = mask_g1
#         mask_filenames = mask_g1_filenames
#     elif group_num == 2:
#         mask = mask_g2
#         mask_filenames = mask_g2_filenames
#     elif group_num == 3:
#         mask = mask_g3
#         mask_filenames = mask_g3_filenames
#     else: 
#         print('<<エラー>> get_labe_dataには1〜3を入力します．')
#         return
    
#     return mask, mask_filenames
...
def simulated_annealing(learn_group, test_group):
    ...
    :returns:  multi_filter_df: マルチチャンネル化の結果
    """
    # 焼きなまし法のパラメータの初期化(初期状態の設定，すべてGRY_)
    feature_array = [[InputFeature(inputfeature_list[0])] * M_LAYER_NUM for i in range(CHANNEL_NUM)]    
    feature_num_array = [[0] * M_LAYER_NUM for i in range(CHANNEL_NUM)]    
    ...
    # マスク画像の取得
    # mask_test, mask_test_filenames = get_mask_data(test_group)
    ...
    acceptCountMax = 120    # 最大アクセプト回数
    # acceptCountMax = 1000    # 最大アクセプト回数
    ...
    if not os.path.exists(multi_filter_csvpath):
        ...
        for i in range(0, CHANNEL_NUM):
            ...
            for j in range(0, M_LAYER_NUM):
                column_name = 'CH{0}_F{1}_num'.format(i+1, j+1)
                multi_filter_df[column_name] = np.NaN
    ...
    while acceptCount < acceptCountMax:
        ...
        # stepKeep = stepMax * (1-(acceptCount / acceptCountMax))
        if stepKeep > 1:
            step = round(stepKeep)
        else:
            step = 1
        ...
        def change_channel():
            ...
            # 変えるフィルタの種類をランダムに変更する(このとき番号は巡回する)
            for i in range(0, CHANNEL_NUM):
                for j in range(0, M_LAYER_NUM):
                    new_ch_num = feature_num_array[i][j] + random.randint(-step, step)
                    if (new_ch_num < 0):
                        new_ch_num = -(new_ch_num + 1)
                    if (new_ch_num >= len(InputFeature)):
                        new_ch_num = new_ch_num % len(InputFeature)
                    new_feature_num_array[i][j] = int(new_ch_num)
                    # print(feature_num_array[i][j], new_ch_num)
                    image_type = InputFeature(inputfeature_list[new_ch_num])
                    new_feature_array[i][j] = image_type

            print('-------- 仮の組み合わせは', end="")
            print(new_feature_num_array,end=" = ")
            ...
            for i in range(0, CHANNEL_NUM):
                for j in range(0, M_LAYER_NUM):
                    f_num = int(new_feature_num_array[i][j])
                    new_feature_array[i][j] = InputFeature(inputfeature_list[f_num])
        ...
        while check_already_exists(multi_folder_name):
            ...
            # ループの回数を数える
            loop_counter += 1
            if loop_counter > acceptCountMax:
                print('<<???>> 指定回以内に未探索の場所へ飛べないため終了します．')
                return multi_filter_df
        ...
        # meanPrecision, meanRecall, meanFmeasure = dnn.test(multi_channel_image_test, label_test, label_test_filenames, multi_folder_path, mask_image=mask_test)  
       ...
       ##
        ...
        if str(index) in multi_filter_df.index.values:
           pass   # 何もしない
        else:
            multi_filter_df.loc[str(index)] = '-' # 新規の行を追加する．このとき'-'ですべて初期化
        # 確率の計算
        if acceptCount > 0:
            try:
...
# %%
def eval_sa(multi_filter_df, test_sa_group, learn_group, test_group):
    """
    評価テストの実行
    ...
    #  テストを行う
    print("原画像についてテスト")
    # dnn.test(image_test, label_test, label_test_filenames, gray_resDir, mask_image=mask_test, overlay_on=True, ori_image=image_test_list)
...
#############
    ...
    # print("ここ1")
    # plt.imshow(multi_channel_image_train[0])
    # plt.show()
    multi_channel_image_test = create_multi_channel(feature_array, test_group) 
    # print("ここ2")
    # plt.imshow(multi_channel_image_test[0])
    # plt.show()
    ...
    # dnn.test(multi_channel_image_test, label_test, label_test_filenames, test_resDir, mask_image=mask_test, overlay_on=True, ori_image=image_test_list) 
    ...
    # 結果をまとめる
    total_res_df = pd.DataFrame(index=['case'], columns=['','precision(G)', 'recall(G)', 'F-measure(G)', '', 'precision(M)', 'recall(M)', 'F-measure(M)'])
    total_res_df.iat[0, 0] = 'gray'
    total_res_df.iat[0, 4] = 'multi'
    ...
    # 平均を求める
    # total_res_df.loc['Average'] = total_res_df.mean()
...
# テストをEVAL_TEST_NUM回だけ行う(必要はなく1回だけで)
for j in range(0, EVAL_TEST_NUM):
    total_res_df = pd.DataFrame(index=['case'], columns=['','precision(G)', 'recall(G)', 'F-measure(G)', '', 'precision(M)', 'recall(M)', 'F-measure(M)'])
    total_res_df.iat[0, 0] = 'gray'
    total_res_df.iat[0, 4] = 'multi'
    for i in range(0,3):
        multi_filter_df = simulated_annealing(LEARN_GROUP[i], TEST_SA_GROUP[i])   # 焼きなましの実行(実行済みならスキップされる)
        res_df = eval_sa(multi_filter_df, TEST_SA_GROUP[i], LEARN_GROUP[i], TEST_GROUP[i])   # 結果のテスト
...
total_res_df_n = pd.DataFrame(index=['case'], columns=['','precision(G)', 'recall(G)', 'F-measure(G)', '', 'precision(M)', 'recall(M)', 'F-measure(M)'])
